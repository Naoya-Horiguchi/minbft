...
func startPeerConnections(replicaID, n uint32, connector api.ReplicaConnector, log messagelog.MessageLog, logger *logging.Logger, requestforward map[uint32]messagelog.MessageLog) error {
	for peerID := uint32(0); peerID < n; peerID++ {
		...
		// TODO: using messagelog for buffering the forwarded requests could
		// make the requests accumulated when the destination replica stops
		// receiving them. Currently that does not happen because we only need
		// to keep at most one request per client. We will need some update when
		// we add support for concurrent client requests.
